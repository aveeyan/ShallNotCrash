# shallnotcrash/landing_site/apt_dat_loader.py
"""
[INTEGRATED - V5]
Loads and processes runway data from FlightGear's apt.dat file.

This loader is adapted from the path_planner module to be self-contained.
It no longer depends on external data models and instead returns a generic
list of dictionaries, making it a portable and independent data source.
"""
import os
import gzip
import shutil
...
class AptDatLoader:
    ...
    def load_runways_in_radius(self, center_lat: float, center_lon: float, radius_km: float) -> List[Dict[str, Any]]:
        """Extracts all runways within a radius, returning a list of dictionaries."""
    ...
    def _parse_runways(self, file_path: str, center_lat: float, center_lon: float, radius_km: float) -> List[Dict[str, Any]]:
        runways = []
        current_airport = "N/A"
    ...
    def _parse_runway_line(self, parts: List[str], airport_code: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            if abs(lat1 - lat2) < 1e-6 and abs(lon1 - lon2) < 1e-6: return None # Ignore zero-length runways
            ...
            # Return a dictionary instead of a custom Runway object
            return {
                "name": name,
