...
from .runway_loader import RunwayLoader, Runway as FlightGearRunway
from .osm_data_handler import OSMDataHandler
from .terrain_analyzer import TerrainAndSafetyAnalyzer
...
class LandingSiteFinder:
    ...
    def __init__(self, config: Optional[SearchConfig] = None):
        ...
        self.analyzer = TerrainAndSafetyAnalyzer(
            self.config.civilian_exclusion_radius_m,
    ...
    def _process_osm_elements(self, elements: List[Dict], origin_lat: float, origin_lon: float, all_nearby_elements: List[Dict], profile: Dict) -> List[LandingSite]:
        ...
        for elem in elements:
            ...
            sites.append(LandingSite(
                ...
                safety_report=safety_report, polygon_coords=simplified_coords,
                surface_type=surface, orientation_degrees=orientation
            ))
        return sites
    ...
    def _process_fg_runways(self, runways: List[FlightGearRunway], origin_lat: float, origin_lon: float, all_nearby_elements: List[Dict], profile: Dict) -> List[LandingSite]:
        ...
        for runway in runways:
            length_m = int(runway.length_ft * 0.3048)
            ...
            sites.append(LandingSite(
                ...
                safety_report=safety_report, polygon_coords=poly_coords, surface_type=runway.surface,
                orientation_degrees=runway.heading, elevation_m=int(runway.elevation_ft * 0.3048)
            ))
        return sites
